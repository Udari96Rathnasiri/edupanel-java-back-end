        ...
        try (Connection connection = pool.getConnection()) {
            ...
            try {
                PreparedStatement stmInsertLecturer = connection.prepareStatement("INSERT INTO lecturer " +
                        "(name, designation, qualification, linkedin) VALUES (?,?,?,?)", Statement.RETURN_GENERATED_KEYS);
                ...
                if (lecturer.getPicture() != null && !lecturer.getPicture().isEmpty()) {
                    PreparedStatement stmUpdateLecturer = connection.prepareStatement
                            ("UPDATE lecturer SET picture = ? WHERE id =?");
